...
echo "t bins  = $7" ## time bins. Options: None, default, short, or an iterable
...
sbatch -J $SLURM_JOB_NAME-redo -o ../shell_logs/%j-$SLURM_JOB_NAME-repeat-of-$SLURM_JOB_ID.out --dependency=afternotok:$SLURM_JOB_ID ~/hop_scripts/parse-r2a2.sh $1 $2 $3 $4 $5 $6 $7
...
## and compare to how many atoms there are to parse. 1 atom = 1 r2 file.
...
r2_files=`ls r2s$DIR | grep "${rd}ps\.csv" | wc -l`
num_tries=1
...
parallel="parallel --delay 0.2 --jobs $SLURM_NTASKS --joblog logs/r2-$1-$2-$3-$4-${rd}ps.log --resume-failed --timeout 300%"
...
do
    ...
    file_out=./r2s$DIR/{1/.}-r2-${rd}ps.csv timebins=$7 " ::: ${ATOMS}${DIR}/*.dat
...
## security against sherlock lags: replicate this job with an "afternotok" dependency
sbatch --dependency=afternotok:$SLURM_JOB_ID -J $SLURM_JOB_NAME-combine -o ../shell_logs/%j-combine-$SLURM_JOB_NAME-$SLURM_JOB_ID.out ~/hop_scripts/combine-bigmem.sh $1 $2 $3 $4 $5 $6 a2
