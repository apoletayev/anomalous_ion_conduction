...
# =============================================================================
# %% compute 1NN network from freud Voronoi objects
# ## This was done once for every plane and the result stored in a 2D matrix of distances
# =============================================================================

# =============================================================================
# %% compute hop correlation function for one plane via pre-defined hop graph
# ## BR_sites: list.
# ## path_lengths: square matrix
# =============================================================================
    
def hop_correlation(plane, data, BR_sites, path_lengths, max_nn_number, 
                    time_range = [-0.05,0.05],verbose=False, timing=True, **kwargs):
    ...
    if plane is not None :
        ...
        s = plane.stoich
        z = plane.z
        new_r_col = 'new_ox_r' if plane.phase == 'beta' else 'new_mg_count'
        old_r_col = 'old_ox_r' if plane.phase == 'beta' else 'old_mg_count'
    else: 
        m = 'M'; T1 = '0'; s = '1.20'; z = '0' ## this line is used for logging
        new_r_col = 'new_ox_r'
        old_r_col = 'old_ox_r'
        print(f'no spec passed, assuming M={m}, T1={T1}K, stoich={s}, z={z}')
        
        
    ## exact path or max path length; default exact; moot for NN=1
    exact_NN=True
    try : exact_NN = bool(kwargs['exact'])
    except : pass

    ## follow; default true
    follow = True
    try : follow = bool(kwargs['follow'])
    except : pass

    ## consecutive; default is False
    consecutive = False
    try : consecutive = bool(kwargs['consecutive'])
    except : pass
        
    # data['old_is_BR'] = data.old_cell.isin(BR_sites).astype(bool)
    
    def calc_path_new(new_cell_s, new_cell_t):
        ''' Connector: calculates distance between new cells in networkx graph g '''
        path_len_new = path_lengths[list(new_cell_s.astype(int)), list(new_cell_t.astype(int))]
        return path_len_new

    def calc_path_old(old_cell_s, old_cell_t):
        ''' Connector: calculates distance between old cells in networkx graph g '''
        path_len_old = path_lengths[list(old_cell_s.astype(int)), list(old_cell_t.astype(int))]
        return path_len_old
    
    def delta_path(path_len_old, path_len_new):
        ''' Connector: zero is equivalent to following'''
        delta_path = path_len_new - path_len_old
        return delta_path
    
    def calc_dt(time_s, time_t):
        ''' Connector: calculates time between distinct hops '''
        dt = time_t - time_s
        return dt
    
    def consec(dt, old_resid_time_t):
        ''' Connector: True if follower hop has been sitting since the leader moved '''
        consec = old_resid_time_t > dt
        return consec

    def distinct_ion(ion_s, ion_t, sources, targets):
        ''' Selector: keeps hops of only distinct ions '''
        di = (ion_s.astype(int) != ion_t.astype(int))
        sources = sources[di]
        targets = targets[di]
        return sources, targets
    
    def exact_path_old(path_len_old,sources, targets):
        ''' Selector: keeps edges where old_cell's are an exact specific distance apart ''' 
        sources = sources[path_len_old == int(max_nn_number)]
        targets = targets[path_len_old == int(max_nn_number)]
        return sources, targets

    def exact_path_new(path_len_new,sources, targets):
        ''' Selector: keeps edges where old_cell's are an exact specific distance apart ''' 
        sources = sources[path_len_new == int(max_nn_number)]
        targets = targets[path_len_new == int(max_nn_number)]
        return sources, targets
    
    def max_path_old(path_len_old,sources, targets):
        ''' Selector: keeps edges where old_cell's are an exact specific distance apart ''' 
        sources = sources[path_len_old <= int(max_nn_number)]
        targets = targets[path_len_old <= int(max_nn_number)]
        return sources, targets

    def max_path_new(path_len_new,sources, targets):
        ''' Selector: keeps edges where old_cell's are an exact specific distance apart ''' 
        sources = sources[path_len_new <= int(max_nn_number)]
        targets = targets[path_len_new <= int(max_nn_number)]
        return sources, targets

    def min_dt(dt, sources, targets):
        ''' Selector: keeps short differences in absolute time '''
        sources = sources[dt > min(time_range)]
        targets = targets[dt > min(time_range)]
        return sources, targets
    
    def source_BR(old_cell_s, sources, targets):
        ''' Selector: keeps sources that start on BR sites '''
        sources = sources[old_cell_s.astype(int) in BR_sites]
        targets = targets[old_cell_s.astype(int) in BR_sites]
        return sources, targets
    
    def follows(path_len_old, path_len_new, sources, targets):
        ''' Selector: keeps hops that have old path == new path, i.e. followers '''
        sources = sources[path_len_old.astype(int) == path_len_new.astype(int)]
        targets = targets[path_len_old.astype(int) == path_len_new.astype(int)]
        return sources, targets
    
    def is_consecutive(dt, old_resid_time_t, sources, targets):
        ''' Selector: checks consecutive-ness of hops. Pair is consecutive if: 
            follower has arrived to its old cell before leader hops'''
        sources = sources[old_resid_time_t >= dt]
        targets = targets[old_resid_time_t >= dt]
        return sources, targets
        
    if timing : 
        print('\nStarting NN={} ({}), {:.2f}-{:.2f} psec'.format(int(max_nn_number),['max', 'exact'][exact_NN], min(time_range),max(time_range)))
        print('following   : {}'.format(['off','on'][follow]))
        print('consecutive : {}'.format(['off','on'][consecutive]))
#        print '\n'
        dg_start = timer()
    
    min_chunk_size = 4e3 ## seems to work for Andrey's 2014 MBP
    if 'min_chunk_size' in list(kwargs.keys()):
        min_chunk_size = kwargs['min_chunk_size']

    ## create graph and connect edges
    dg = DeepGraph(v=data.copy(deep=True))
    
    ## option to have exact or inexact 
    connectors=[calc_path_old,calc_path_new,calc_dt,delta_path,consec]
    selectors=[distinct_ion,min_dt]
    tfs  = ['rev_hop','old_is_BR','new_resid_time','old_cell','new_cell','ion', \
            'old_resid_time', new_r_col, old_r_col]
    ntfs = ['dt']
    
    ## old path length: max or exact. 'exact' keyword arg
    ## insert at start as this prunes possible edges fast
#    selectors.insert(0,[max_path_new,exact_path_new][exact_path]) ## This is for following
    selectors.insert(0,[max_path_old,exact_path_old][exact_NN])
    
    ## consecutive
    if consecutive: 
        selectors.append(is_consecutive)
        ntfs.append('old_resid_time')
    
    ## following: path lengths moot if exact
    if follow: 
        selectors.append(follows)
#        if exact_NN: 
#            ntfs.append('path_len_old'); ntfs.append('path_len_new')
    
    dg.create_edges_ft(('time',max(time_range)),
                    connectors, selectors, transfer_features=tfs,
                    no_transfer_rs=ntfs,
                    min_chunk_size=int(min_chunk_size), verbose=verbose,
                    logfile='./production/graph_logs/{} {}K {} {}NN {}-{}ps.txt'.format(
                            m,T1,z,int(max_nn_number),min(time_range),max(time_range)))
    
    dg.append_cp() ## consolidation takes extra work later when agg'ing by component
    dg.e.rename(columns={'ft_r':'dt'},inplace=True)

    if timing:
        dg_end = timer()
        print('Graph complete in {:.2f} sec\n'.format(dg_end-dg_start))
#        print '{}/{} edges start at BR sites'.format(dg.e.old_is_BR_s.sum(),len(dg.e))
    
    ## ===========================
    ## extract and output averages
    ## ===========================
    
    dg_con = dg.v.loc[dg.v.cp != 0].copy()
    dg_dis = dg.v.loc[dg.v.cp == 0].copy()
    
    dg_s = np.array(dg.e.index.get_level_values('s').unique()) ## indices of sources
    dg_t = np.array(dg.e.index.get_level_values('t').unique()) ## indices of targets
    dg_ss = np.array(list(set(dg_s)-set(dg_t))) ## indices of sources that are not targets
    dg_tt = np.array(list(set(dg_t)-set(dg_s))) ## indices of targets that are not sources
    dg_st = np.array(list(set(dg_s) & set(dg_t))) ## indices of sources that are also targets
    
    ## subdivide graph 
    sources = data.iloc[dg_s]
    targets = data.iloc[dg_t]
    ss      = data.iloc[dg_ss]
    st      = data.iloc[dg_st]
    tt      = data.iloc[dg_tt]
    
    keys = ['con', 'dis', 's', 't', 'ss', 'st', 'tt']
    dists = {'con':dg_con, 'dis':dg_dis, 's':sources, 't':targets,
             'ss':ss, 'st':st, 'tt':tt}
    
    ## calc P(rev) by BR and a-BR
    exp_p_rev_BR =  data.query('old_is_BR').rev_hop.mean() ## population-average P(rev, BR)
    exp_p_rev_aBR = data.query('not old_is_BR').rev_hop.mean() ## population-average P(rev,aBR)
    
    fraction_BR = {k:dists[k].old_is_BR.mean() for k in keys}
    
    expecteds = {k: fraction_BR[k] * exp_p_rev_BR + (1.-fraction_BR[k]) * exp_p_rev_aBR for k in keys}
    expecteds_BR  = {k: exp_p_rev_BR for k in keys}
    expecteds_aBR = {k:exp_p_rev_aBR for k in keys}
    
    actuals = {k:dists[k].rev_hop.mean() for k in keys}
    actuals_BR  = {k:dists[k].query('old_is_BR').rev_hop.mean() for k in keys}
    actuals_aBR = {k:dists[k].query('not old_is_BR').rev_hop.mean() for k in keys}

    actual_sum = {k:int(dists[k].rev_hop.sum()) for k in keys}
    actual_sum_BR  = {k:int(dists[k].query('old_is_BR').rev_hop.sum()) for k in keys}
    actual_sum_aBR = {k:int(dists[k].query('not old_is_BR').rev_hop.sum()) for k in keys}
    
    prevalences = {k: len(dists[k]) for k in keys}
    prevalences_BR  = {k:len(dists[k].query('old_is_BR')) for k in keys}
    prevalences_aBR = {k:len(dists[k].query('not old_is_BR')) for k in keys}
    
    ## quick output
    ## TODO: differentiate between count and edge count. not the same.
    if timing: 
        key = 'ss'
        print('P(onw, ss)      = {:5.2f}%, expect {:.2f}%, ({:d}/{:.1f}) {} count'.format(
                (1.-actuals[key])*100,(1.-expecteds[key])*100, 
                prevalences[key] - actual_sum[key], prevalences[key] * (1.-expecteds[key]), prevalences[key]))
        print('P(onw, ss, BR)  = {:5.2f}%, expect {:.2f}%, ({:d}/{:.1f}) {} count'.format( 
                (1.-actuals_BR[key])*100,(1.-expecteds_BR[key])*100,
                prevalences_BR[key] - actual_sum_BR[key], prevalences_BR[key] * (1.-expecteds_BR[key]), prevalences_BR[key]))
        print('P(onw, ss, aBR) = {:5.2f}%, expect {:.2f}%, ({:d}/{:.1f}) {} count'.format( 
                (1.-actuals_aBR[key])*100,(1.-expecteds_aBR['s'])*100,
                prevalences_aBR[key] - actual_sum_aBR[key], prevalences_aBR[key] * (1.-expecteds_aBR[key]), prevalences_aBR[key]))
    
    stats = [actuals, expecteds, prevalences, actuals_BR, expecteds_BR, prevalences_BR, \
               actuals_aBR, expecteds_aBR, prevalences_aBR]
...
# =============================================================================
# %% calculate numbers of hops by size of component
# ## TODO: this is probably doable with DeepGraph.partition_graph()
# =============================================================================

def hops_by_component_size(vertices):
    ''' input: vertices of a deepgraph with cp appended, where 'ion' is a column.
        return: sizes=component sizes, lens=numbers of hops by component size'''
    
    cpcounts  = vertices.groupby('cp').ion.count()
    
    vertices['onward']=1.-vertices.rev_hop
    onwcounts=vertices.groupby('cp').onward.sum()
    
    cpcounts = pd.concat([cpcounts,onwcounts],axis=1)
    #print 'largest component: {}'.format(cpcounts.max())

    sizes = list(cpcounts.ion.unique())
    lens  = list()
    onws  = list()
    for size in sizes: 
        components = cpcounts.query('ion == @size')
        lens.append(components.ion.sum())
        onws.append(components.onward.sum())
#        print size, lens[-1], onws[-1]
    
    sizes = np.array(sizes).astype(float)
    lens = np.array(lens).astype(float)
    onws = np.array(onws).astype(float)
    
    return sizes, lens, onws
